...
pdfjsLib.GlobalWorkerOptions.workerSrc =
  ...
  $(document).bind('pagerendered', function (e) {
		...
		$('.drag-drop.can-drop').each(function( index ) {
  		  var x = parseFloat($(this)[index].dataset.x);
  		  var y = parseFloat($(this)[index].dataset.y);
		  ...
		  curPage = pageChange;	
  			val = {"descrizione": descrizione, "posizioneX":posizioneX,   "posizioneY":posizioneY, "valore":valore,"currentPage":curPage};
			saveLocation.push(val);
  	  	});
		saveLocation = [... new Set(saveLocation)];
		 $('#parametriContainer').empty();
		renderizzaPlaceholder(currentPage, parametri,saveLocation);	
	});
  
  function renderizzaPlaceholder(currentPage, parametri,saveLocation){
		  var maxHTMLx = $('#the-canvas').width();
			var maxHTMLy = $('#the-canvas').height();
			...
			if(saveLocation!=null){
				for(let val of saveLocation){
					if(val.currentPage == currentPage){
						isValid= false;
						classStyle = "drag-drop can-drop";		
						var posY = maxHTMLy - val.posizioneY;
						$(".parametriContainer").append('<div class="'+classStyle+'" data-id="-1" data-page="'+val.currentPage+'" data-toggle="'+val.valore+'" data-valore="'+val.valore+'" data-x="'+val.posizioneX+'" data-y="'+posY+'" style="transform: translate('+val.posizioneX+'px, '+posY+'px); display:'+display+';background-image: url('+'https://picsum.photos/200'+');width:200px;height:200px">  <span class="circle"></span><span class="descrizione">'+val.descrizione+' </span></div>');
						break;
					}
				}
			}
			if(isValid){
				for (i = 0; i < parametri.length; i++) {
					...
					$(".parametriContainer").append('<div class="'+classStyle+'" data-id="-1" data-page="'+page+'" data-toggle="'+valore+'" data-valore="'+valore+'" data-x="'+x+'" data-y="'+y+'" style="transform: translate('+x+'px, '+y+'px); display:'+display+';background-image: url('+'https://picsum.photos/200'+');width:200px;height:200px">  <span class="circle"></span><span class="descrizione">'+param.descrizione+' </span></div>');
				}
			}
			...
			$(".parametriContainer").append('<ul id="pager" class="pager" style="transform: translate(' + x + 'px, ' + y + 'px); width:200px;">' +
			'<li onclick="changePage(' + (currentPage === 1) + ', -1)" class="page-item ' + (currentPage === 1 ? 'disabled' : '') + '"><span>«</span></li>' +
			'<li onclick="changePage(' + (currentPage === totalPages) + ', 1)" class="page-item ' + (currentPage === totalPages ? 'disabled' : '') + '" style="margin-left:10px;"><span>&raquo;</span></li>' +
			'</ul>');
	 }
	  ...
	  function changePage(disabled, delta) {
		//  if(disabled){
		// 	return;	 
		//  }
		...
		if (disabled) {
		  ...
		  pdfs.getPage(currentPage).then(function (page) {
		  // Render the new page
			var scale = 1.0;
			var viewport = page.getViewport(scale);
			...
			page.render(renderContext).then(function () {
			  // Trigger the "pagerendered" event when the page is rendered
			  $(document).trigger('pagerendered');
			});
		  });
